...
In the fourth chapter of the tutorial on [building an end-to-end dapp on Move](../build-e2e-dapp.mdx), you will be learning to fetch data from chain.
...
To fetch data from chain, we can use the [Move TypeScript SDK](../../sdks/ts-sdk.mdx). The SDK provides classes and functions for us to easily interact and query the Move chain.

To get started:

1. Stop the local server if running.
2. In the `client` directory, run: `npm i @aptos-labs/ts-sdk`
3. In the `App.tsx` file, import the `Move` class like so:

```tsx filename="App.tsx"
import { Move } from "@aptos-labs/ts-sdk";
```

The TypeScript SDK provides us with a `Move` class which is the main entry point into Move's API. By initializing `Move` we can query the Move chain.

<Callout type="info">
Read more about the [`Move`](../../sdks/ts-sdk.mdx) class in the Move TypeScript SDK docs.
</Callout>

1. In the `App.tsx` file, add:

```tsx filename="App.tsx"
const aptos = new Move();
```

This will initialize a `Move` instance for us.

<Callout type="info">
By default, `Move` will interact with the `devnet` network, to set up a [different network](../../guides/system-integrators-guide.mdx#choose-a-network), we can use `AptosConfig` class.

```tsx filename="App.tsx"
import { Move, AptosConfig, Network } from "@aptos-labs/ts-sdk";

const aptosConfig = new AptosConfig({ network: Network.MAINNET });
const aptos = new Move(aptosConfig);
```
